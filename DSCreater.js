...
function extractfromValueBlock(block) {
  //This function returns the information from the inner VALUE block
}
...
/*
    In order to find the defaul values of the parameters, we need to find
...
function valuesOfModuleParameters(module_data) {
  let attributes = findBlocks(module_data, "ATTRIBUTE");
  let attributeInstances = findBlocks(module_data, "ATTRIBUTE_INSTANCE");
  let values = [];
  attributeInstances.forEach((instance) => {
    ...
    for (let index = 0; index < attributes.length; index++) {
      ...
      let name = nameOf(attribute);
      if (name === nameOf(instance)) {
        let value;
        // identify what type of parameter it is before extracting the value
        switch (valueOf(attribute, "TYPE")) {
          case "ENUMERATION_VALUE":
            let set = valueOf(instance, "SET");
            let option = valueOf(instance, "STRING_VALUE");
            value = `$${set}:${option}`;
            break;
          case "FLOAT":
